```js
const greeter = () => {
    console.log('hi there, im tim');
}

greeter();
```

### student and developer who likes dark mode and lowercase

- 👨‍💻 working on [???][working_project]
- 🧠 learning [react][react]
- 👅 languages [javascript][javascript] , [typescript][typescript] , [node][node] , [python][python] , [c][c] & [c++][cpp]
- 🛠 tools [linux][linux] , [vscode][vscode] , [git][git] & [github][github]

🐦 [twitter][twitter] **|** 
🟥 [reddit][reddit] **|** 
📦 [npm][npm] 

---

Inspired by [federicocotogno][gh_federicocotogno] & [bradgarropy][gh_bradgarropy]

---

<!-- LINKS -->
[working_project]: https://github.com/t-holubowicz

[react]: https://reactjs.org/
[javascript]: https://developer.mozilla.org/en-US/docs/Web/JavaScript
[typescript]: https://www.typescriptlang.org/
[node]: https://nodejs.org/
[python]: https://www.python.org/
[c]: https://www.cprogramming.com/
[cpp]: https://www.w3schools.com/cpp/
[linux]: https://www.linux.org/
[vscode]: https://code.visualstudio.com/
[git]: https://git-scm.com/
[github]: https://github.com/

[twitter]: https://twitter.com/tholubowicz
[reddit]: https://www.reddit.com/u/tholubowicz
[npm]: https://www.npmjs.com/~tholubowicz

[gh_me]: https://github.com/t-holubowicz
[gh_federicocotogno]: https://github.com/federicocotogno
[gh_bradgarropy]: https://github.com/bradgarropy
